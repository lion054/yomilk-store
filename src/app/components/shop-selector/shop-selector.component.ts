            ...
            <div class="mt-3 p-3 bg-white/10 backdrop-blur-sm rounded-xl">
              <div class="flex items-center gap-2">
                <div class="w-8 h-8 bg-white/20 rounded-lg flex items-center justify-center">
                  ...
                  @if (isLoadingLocation) {
                    <svg class="animate-spin" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  @if (userAddress) {
                    <p class="text-white text-sm font-medium truncate">{{ userAddress }}</p>
                  }
                  @if (!userAddress && !isLoadingLocation && !locationError) {
                    <p class="text-white/80 text-sm">
                      <button (click)="detectLocation()" class="underline hover:no-underline">Detect my location</button>
                    </p>
                  }
                  @if (isLoadingLocation) {
                    <p class="text-white/80 text-sm">Detecting location...</p>
                  }
                  @if (locationError && !userAddress) {
                    <p class="text-white/80 text-xs">{{ locationError }}</p>
                  }
                </div>
                @if (userAddress || locationError) {
                  <button
              ...
              : shop.isActive
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    ...
                    @if (shop.badge === 'featured') {
                      <span class="text-[10px] px-1.5 py-0.5 bg-amber-100 text-amber-700 rounded-full font-bold">Featured</span>
                    }
                    @if (shop.badge === 'new') {
                      <span class="text-[10px] px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded-full font-bold">Coming Soon</span>
                    }
                  </div>
                  <p class="text-xs text-gray-500 truncate mt-0.5">{{ shop.address }}</p>
                  <!-- Status & Distance -->
                  <div class="flex items-center gap-3 mt-2">
                    @if (shop.isActive && isShopOpen(shop)) {
                ...
                @if (shop.id === activeShop?.id) {
                  <div class="w-6 h-6 bg-[#42af57] rounded-full flex items-center justify-center shrink-0">
